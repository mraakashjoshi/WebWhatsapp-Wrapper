...
{
id: "NumQuery",
conditions: module => (module.queryExist ? module.queryExist : null)
},
//{ id: "Cmd", conditions: (module) => (module.default && module.default.openChatFromUnread) ? module : null }, //old
{
id: "Cmd",
conditions: module => (module && module.Cmd ? module.Cmd : null)
}, //2021-12-17
{
id: "ReadSeen",
...
// Change for compatibility with WhatsApp
metadata.participants.models =
metadata.participants.models || metadata.participants._models;
...
const teste = Store.FindChat.findChat(idUser).then(chatid => {
console.log(teste);
...
mc.processAttachments([{ file: mediaBlob }, 1], chat, 1)
.then(() => {
...
await WAPI.createMessageLink(chatid, "hi");
let activeChat = Store.Chat.getActive();
console.log("使用当前激活对象发送", activeChat);
send(activeChat);
});
};
...
try {
let { __x_img, __x_imgFull } = window.Store.ProfilePicThumb._index[
item.id._serialized
];
...
let area_code = Store2.NumberInfo.findCC(item.id.user) || "no";
return {
...
if (window.WAPI.isMultiDevice === undefined) {
try {
let res = await window.mR
.findModule("QueryExist")[0]
.QueryExist("+38670531653@c.us");
window.WAPI.isMultiDevice = res !== undefined;
return window.WAPI.isMultiDevice;
} catch (error) {
window.WAPI.isMultiDevice = false;
return false;
}
} else {
return window.WAPI.isMultiDevice;
...
// 常规load-user,后发送
window.WAPI.createMessageLinkSend = async (number, text) => {
let timer = null;
await window.WAPI.createMessageLink(number, text);
return new Promise(resolve => {
timer = setInterval(() => {
// let target = document.querySelector("._4sWnG")
let target = document.querySelector("[data-icon='send']");
if (target) {
target.click();
clearInterval(timer);
resolve();
...
// 获取聊天对象以及通讯录
WAPI.getAllChatAndPhone = () => {
let iframeGroup = WAPI.getAllChats()
...
let phoneGroup = WAPI.getMyContacts()
.map(item => {
return {
number: item.id.user,
...
.forEach(row => {
let target = iframeGroup.find(item => item.number === row.number);
if (target) {
target.name = row.name;
} else {
iframeGroup.push(row);
}
});
